...
makeCacheMatrix <- function(x = matrix()) {
  i <- NULL
  set <- function(y) {
    x <<- y ## Value of the matrix in the cache
    i <<- NULL## If there is no inverse for this matrix, reset the value to NULL
  }
  get <- function() x
  seti <- function(inverse) i <<- inverse  ## Get the inverse value from cache
  geti<- function() i
